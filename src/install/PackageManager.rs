...
impl PackageManager {
    ...
    pub fn apply_config_version_defaults(
        ...
        if !already_applied {
            ...
            // `0` explicitly disables the filter. Normalize to `None` so
            // callers that check `.is_some()` don't needlessly request
            // extended manifests.
            if self.options.minimum_release_age_ms == Some(0.0) {
                self.options.minimum_release_age_ms = None;
            }
        }
