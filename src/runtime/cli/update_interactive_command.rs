...
/// Signal-handler machinery for the multi-select prompt. The prompt hides
/// the cursor and enables mouse tracking on entry; if the process is killed
/// by SIGINT/SIGTERM before the `scopeguard::defer!` can run (or on Windows,
/// before the byte-3 graceful path fires), the terminal is left with an
/// invisible cursor and mouse tracking on. See #30890.
///
/// On Unix we install a SIGINT/SIGTERM handler that writes the ANSI
/// restore sequence directly to stdout via `write(2)` (async-signal-safe),
/// restores the termios via the existing `uv_tty_reset_mode` atexit hook,
/// and `_exit`s with the conventional `128 + signum`.
///
/// On Windows we install a `SetConsoleCtrlHandler` that writes the same
/// sequence and returns `TRUE` so the default handler skips its
/// `ExitProcess`; we call `ExitProcess` ourselves with `STATUS_CONTROL_C_EXIT`.
///
/// The prompt-entry code builds a [`PromptSignalGuard`]; its `Drop` removes
/// the handler so the CLI's normal signal semantics resume after the prompt.
mod prompt_signal {
    #[cfg(unix)]
    use core::sync::atomic::{AtomicI32, Ordering};

    /// Cursor show + mouse-tracking off + SGR-extended-mouse off + CRLF.
    /// Writing CRLF (not just LF) is intentional: the slave TTY may still
    /// be in raw mode when the signal fires (termios restore happens inside
    /// the handler *after* this write), so we must emit the carriage return
    ...
    #[cfg(unix)]
    unsafe extern "C" {
        // libuv entry point already linked by `Bun__ttySetMode`'s atexit
        // hook (see src/jsc/bindings/wtf-bindings.cpp). tcsetattr is not
        // strictly async-signal-safe per POSIX, but libuv + the existing
    ...
    extern "C" fn handler(sig: i32) {
        ...
        // SAFETY: no preconditions; internal atomic spinlock + tcsetattr
        // on a saved termios snapshot. Call from signal handler mirrors
        // libuv's own `onExitSignal` in src/jsc/bindings/c-bindings.cpp.
        unsafe {
            uv_tty_reset_mode();
        }
        // Conventional `128 + signum` exit status for death-by-signal.
        // _exit is async-signal-safe and does not run atexit (which would
        // try to restore things we've already restored).
    ...
    /// but cheap insurance) doesn't uninstall the handler the outer call
    /// still relies on.
    #[cfg(unix)]
    ...
    pub fn install() -> Guard {
        ...
        #[cfg(windows)]
        {
            ...
            // covered by unsetting ENABLE_PROCESSED_INPUT (byte 3 reaches
...
impl UpdateInteractiveCommand {
    ...
    fn prompt_for_updates(
        ...
        // `prompt_signal` above for the full rationale.
        let _signal_guard = prompt_signal::install();
