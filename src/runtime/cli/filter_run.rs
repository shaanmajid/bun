...
pub(crate) fn run_scripts_with_filter(
    ...
    if !ctx.debug.loaded_bunfig {
        ...
        // `Arguments::parse` re-applies the flags after the config load
        // (Arguments.rs:1374-1411); this fallback runs after `parse` returns, so
        // snapshot the CLI-applied values and restore them afterward, preserving
        // that precedence. Only the fields this path actually consumes need it:
