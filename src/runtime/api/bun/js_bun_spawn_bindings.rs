...
pub(crate) fn spawn_maybe_sync<const IS_SYNC: bool>(
    ...
    //
    // `owner` is this freshly-boxed subprocess (stable address), stored erased on
    // each `MaxBuf`. On overflow the `BufferedReaderParentLink::on_max_buffer_overflow`
    // handler reads it back out and casts it to `Subprocess<'static>`. The
    ...
    // back-pointer never outlives the subprocess.
    {
        ...
        let mut mb = None;
        // SAFETY: `owner` is live for the subprocess's lifetime — see the note above.
        unsafe {
