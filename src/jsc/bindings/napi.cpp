...
// napi enum. Returns false for DataView or any non-typed-array type.
    ...
    size_t* byte_offset)
{
    ...
    if (type) {
        // `Float16Array` is a typed array but has no `napi_typedarray_type` value; like Node,
        // leave `*type` unwritten in that case rather than failing the whole call.
        getNAPITypeFromJSType(view->type(), type);
    }
    if (length) {
        *length = view->length();
    }
