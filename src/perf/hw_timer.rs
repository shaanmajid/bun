...
/// clock.
///
/// PORT NOTE: diverges from the Zig spec, which returns the leaf-0x15 value
/// unconditionally. Inside a guest the TSC the OS hands out may be
/// scaled/emulated, so leaf 0x15 (host crystal info leaked through the VMM)
/// need not match the rate `rdtsc` actually ticks at — on some GCP/KVM hosts
/// it is off by ~2.8×, which skews every deadline derived from a clock
/// calibrated with it. The only CPUID source trusted under a hypervisor is the
/// timing leaf the hypervisor itself publishes; otherwise we fall back to the
/// OS clock, which the guest kernel already calibrates correctly.
