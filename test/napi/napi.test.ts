...
describe("cleanup hooks", () => {
  ...
  describe("napi_get_typedarray_info / napi_get_dataview_info", () => {
    it("reports the view's real byteOffset so that arraybuffer-base + byte_offset == data", async () => {
      ...
      // A Float16Array (no napi enum value) still succeeds when type is not requested.
      // Skipped when the comparison Node predates unflagged Float16Array.
      if (float16) {
        expect(output).toContain("PASS: get_typedarray_info(float16, type=NULL)");
      } else {
        expect(output).toContain("SKIP: get_typedarray_info(float16, type=NULL)");
      }
