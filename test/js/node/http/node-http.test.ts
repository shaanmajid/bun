...
// https://github.com/oven-sh/bun/issues/31318 (and #20433)
// IncomingMessage.rawHeaders must preserve the wire case of header names (Node docs:
// "Header names are not lowercased, and duplicates are not merged"). IncomingMessage.headersDistinct
// must be a per-name array dictionary parallel to `headers`.
describe("IncomingMessage header views", () => {
  async function receiveRequest(payload: string) {
    ...
    const server = createServer(req => {
      resolve(req);
      // Don't respond — the request handler only inspects headers. The test ends the
      // socket itself so the server can be closed cleanly.
    });
